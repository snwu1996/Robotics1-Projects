...
# ------------------------------------ MEASURE THESE VALUES ON THE DOBOT
...
class DobotKinematics:
	...
	def anglesFromCoordinates(self, x, y, z):
		...
		self._debug('q1', q1)
		# Delete
		self._debug('lengthRearSquared',lengthRearSquared)
		self._debug('lengthFrontSquared',lengthFrontSquared)
		self._debug('hypotenuseSquared',hypotenuseSquared)
		self._debug('lengthRearArm',lengthRearArm)
		self._debug('hypotenuse',hypotenuse)
		acos_in = (lengthRearSquared - lengthFrontSquared + hypotenuseSquared) / (2.0 * lengthRearArm * hypotenuse)
		self._debug('acos_in',acos_in)
		# Delete \
		q2 = math.acos(acos_in)
		self._debug('q2', q2)
